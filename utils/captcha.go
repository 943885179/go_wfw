...
//ConfigJSONBody jsonos request body.
type ConfigJSONBody struct {
	ID            string                       `jsonos:"id" form:"id"`
	CaptchaType   DriverType                   `jsonos:"captchaType" form:"captchaType"`
	VerifyValue   string                       `jsonos:"verifyValue" form:"verifyValue"`
	DriverAudio   *base64Captcha.DriverAudio   `jsonos:"driverAudio" form:"driverAudio"`
	DriverString  *base64Captcha.DriverString  `jsonos:"driverString" form:"driverString"`
	DriverChinese *base64Captcha.DriverChinese `jsonos:"driverChinese" form:"driverChinese"`
	DriverMath    *base64Captcha.DriverMath    `jsonos:"driverMath" form:"driverMath"`
	DriverDigit   *base64Captcha.DriverDigit   `jsonos:"driverDigit" form:"driverDigit"`
}

func (c *Captcha) Default() (string, string, error) {
	conf := ConfigJSONBody{
		CaptchaType: DIGIT,
	}
	return c.NewCaptcha(&conf)
}

//NewCaptcha 创建新的验证码 base64Captcha create http handler
func (*Captcha) NewCaptcha(c *ConfigJSONBody) (id string, base64 string, err error) {
	//初始化赋值
...
func (c *Captcha) Set(id, value string) { //赋值
	base64Captcha.DefaultMemStore.Set(id, value)
}
func (c *Captcha) Get(id string, clear bool) string { //取值

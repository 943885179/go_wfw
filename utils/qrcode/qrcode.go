...
func NeWQrCodes(context, qrPath string, size int) error {
...
func NeWQrCodeImgs(context, qrPath, avatarPath string, size int) error {
	qrCode, err := qr.Encode(context, qr.M, qr.Auto)
	if err != nil {
		return err
	}
	...
	//读取中间图片
	avimg, err := img.ImageResizeImg(avatarPath, size/5, size/5, 1)
	//居中设置
...
//ReadQrCode 读取二维码
func ReadQrCodes(qrPath string) (result string, err error) {

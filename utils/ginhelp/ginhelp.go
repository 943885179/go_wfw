...
//Resp api接口返回实体
type Resp struct {
	Code   RespCode    `jsonos:"code"`   //编码
	Msg    string      `jsonos:"msg"`    //消息
	Method string      `jsonos:"method"` //请求方式
	URL    string      `jsonos:"url"`    //请求地址
	IP     string      `jsonos:"ip"`     //请求ip
	Result interface{} `jsonos:"result"` //返回值
}

type gomicroErrResp struct {
	ID     string `jsonos:"id"`
	Code   int    `jsonos:"code"`
	Detail string `jsonos:"detail"`
	Status string `jsonos:"status"`
}
...
func (r Resp) APIResult(c *gin.Context, code int, result interface{}) {
	...
	r.IP = c.ClientIP()
	/*str, _ := jsonos.MarshalIndent(resp, "", "   asd ")
...
func TokenResp(token string) (resp basic.LoginResp, err error) {
	//configs.Jwt.Token = c.Request.Header.Get("token")

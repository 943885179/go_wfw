...
const (
	...
	fileName string = "wfw.log" //日志文件
	//错误日志回收
	errlogMaxAge       time.Duration = time.Hour * 24 * 3 //日志自动回收时间
	errlogRotationTime time.Duration = time.Hour          //日志切割时间
	//普通日志回收
	logMaxAge       time.Duration = time.Hour * 24 //日志自动回收时间
...
func GetHook() *lfshook.LfsHook {
	...
	writer, err := rotatelogs.New(
		...
		rotatelogs.WithMaxAge(errlogMaxAge),             // 文件最大保存时间
		rotatelogs.WithRotationTime(errlogRotationTime), // 日志切割时间间隔
	)
	if err != nil {

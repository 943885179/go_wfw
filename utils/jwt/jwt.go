...
type Jwt struct {
	Secret  string        `jsonos:"secret"`  //jwt加密字段
	TimeOut time.Duration `jsonos:"timeOut"` //过期时长
	Iss     string        `jsonos:"-"`       //用户
	Token   string        `jsonos:"token"`   //token
	Data    interface{}   `jsonos:"data"`    //携带的其他信息
}
...
func (t *Jwt) ParseToken(resp interface{}) (err error) {
	...
	//fmt.Println(claim["data"])
	//return mapstructure.Decode(claim["data"], resp)//通过这个转化好像有点问题
	bt, _ := json.Marshal(claim["data"])
	return json.Unmarshal(bt, resp)

}
func main() {
	//TokenUser 用户基本信息（展示用）
	type TokenUser struct {
		Name   string `jsonos:"name"`
		Avatar string `jsonos:"avatar"`
		Email  string `jsonos:"email"`
	}

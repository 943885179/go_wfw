...
type DbConfig struct {
	DriverType DriverType `jsonos:"driverType"` //驱动类型（这个是我自定义的）
	Server     string     `jsonos:"server"`     //服务器
	Port       int        `jsonos:"port"`       //端口
	User       string     `jsonos:"user"`       //用户名
	Password   string     `jsonos:"password"`   //密码
	Database   string     `jsonos:"database"`   //数据库
	Source     string     `jsonos:"source"`     //完整连接（优先读取）
	Sources    []string   `jsonos:"sources"`    //完整连接（优先读取）多个，读写分离
	IsDebug    bool       `jsonos:"isDebug"`    //是否为调试模式
	//DB         *sql.DB    `jsonos:"-"`          //dbdao
}

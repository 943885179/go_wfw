...
type NacOsServerConfig struct {
	Scheme      string `jsonos:"scheme"`       //the nacos server scheme
	ContextPath string `jsonos:"context_path"` //the nacos server contextpath
	IpAddr      string `jsonos:"ip_addr"`      //the nacos server address
	Port        uint64 `jsonos:"port"`         //the nacos server port
}
